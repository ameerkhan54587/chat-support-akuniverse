...
function generateSnippet(config) {
  ...
  if (config.telegramBotId) {
    configObj.telegramBotId = config.telegramBotId;
  }
  configObj.aiEnabled = config.aiEnabled;
  if (config.aiPrompt.trim()) {
    configObj.aiPrompt = config.aiPrompt.trim();
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        <label className={labelClass}>{t('settings.widget.telegramBotId')}</label>
        <input
          type="text"
          value={config.telegramBotId}
          onChange={(e) => update('telegramBotId', e.target.value)}
          className={inputClass}
          placeholder={t('settings.widget.telegramBotIdPlaceholder')}
        />
        <p className={hintClass}>{t('settings.widget.telegramBotIdHint')}</p>
      </div>

      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <span className="text-sm font-medium text-gray-700">{t('settings.widget.aiEnabled')}</span>
            <p className="text-xs text-gray-500">{t('settings.widget.aiEnabledHint')}</p>
          </div>
          <input
            type="checkbox"
        ...
        {config.aiEnabled && (
          <textarea
            ...
            placeholder={t('settings.widget.aiPromptPlaceholder')}
          />
        )}
      </div>
